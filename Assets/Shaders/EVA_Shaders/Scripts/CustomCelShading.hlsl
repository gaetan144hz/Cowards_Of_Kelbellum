...
float3 CustomLightHandling(CustomLightingData d, Light light) {
    ...
    float stepSize = 1.0f / d.steps;
    float diffuseStep = floor(diffuse / stepSize);
    diffuseStep = diffuseStep * stepSize;
    float specularStep = floor(specular / stepSize);
    specularStep = specularStep * stepSize;

    float3 color = d.albedo * radiance * (diffuseStep + specularStep);
